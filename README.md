...
1. Set up Tools
2. Set up Files
3. Set up ConfigFile for alignment only
4. Launch Alignment
...
Download from gencode or ENSEMBL GTF/GFF3 for annotations.  
Can be usefull to create right now a sqlite annotation database.  
Call R in command line and do:  

```R
library(GenomicFeatures)
txdb <- makeTxDbFromGFF('gencode.vM15.primary_assembly.annotation.gtf')
saveDb(txdb,file ='gencode.annotation.sqlite')
```
This will create gencode.annotation.sqlite file.  

Also you need to create a file containing the size of genes.(counting exons only). Modify output name inside script -hard coded.

```shell
Rscript geneSize.R --file=Path/To/GTF/Annotation
```
This will create gencode.annotation.sqlite file.  

Download also fasta sequence of the genome of interest. (Here we use PRIM_ASSEMBLY from GENCODE R25 based on ENSEMBL 85 for Human and GENCODE M15 based on ENSEMBL 90 for Mouse.  
...
```shell
STAR --runMode genomeGenerate --genomeDir ../GRCh38_PRIM_GENCODE_R25/ --genomeFastaFiles data/ref/genome/GRCh38_PRIM_GENCODE_R25/GRCh38.primary_assembly.genome.fa --runThreadN 28 --sjdbGTFfile data/ref/genes/GRCh38_PRIM_GENCODE_R25/gencode.v25.primary_assembly.annotation.gtf
```
...
Also you need to Generate the BWA index for bwa (that's for ChipSeq) :  
```shell
bwa index -a bwtsw genome.fa
```
This creates a collection of files used by BWA to perform the alignment.

Mappability script to run also for (ChipSeq). Need to modify path to files inside the script.  Run first :  

```shell
./mappability.sh
```
Then go into the current genome dir and execute in shell the following command lines :  

```shell
cat GRCm38.primary_assembly.genome_50.wig |awk 'OFS="\t"{if($2==1){print chr,$1,$1+span,$2 >>chr".wig"};if(sub(/chrom=/, "", $2)){chr=$2};span=substr($4,6)}'
mkdir 50mer_wig_file
mv ./*wig ./50mer_wig_file/
cd ./50mer_wig_file/
rename 's/\.wig$/\.map/' *.wig

for i in *.map; do sed -i -r 's/.+/chr&/' $i; done
#sed -i -r 's/chrMT/chrM/' hg38_50mer_MT.map
```

This creates a file called mappability50.tsv and your done.

Last step  : 

Download the transcripts from gencode for your genome version and then inside a dir called transcriptome for instance,
you will create indexes for Kallisto and Salmon as follows : 

```shell  
salmon index -t gencode.vM15.transcripts.fa.gz -i gencode.m15.transcripts.index --type quasi -k 31  

kallisto index --i transcripts_kallisto_index.gencode.v15.idx gencode.vM15.transcripts.fa.gz
...
Plot are automatically created in the ${PATH_TO_DATA}/[DIR_NAME]/pathway . It gives you enriched ontology terms for the genes which are differentially expressed using a cutoff of | log2FC | > 1.5.  
...
## Alternative Splicing Analysis V2
